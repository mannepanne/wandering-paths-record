...
- [2026-09-12 — Review output is budgeted in lines, and the style rule defines a failure mode rather than banning words](./2026-09-12-plain-review-output.md) — why the synthesis templates went from three sub-bullets per finding to one line, why praise is capped at three sentences and the per-reviewer count block is gone, why the writing rule defines mannered prose as a mechanism instead of a banned-word list, and why the essentials are repeated inline in `.claude/CLAUDE.md` and the agent contract rather than living only behind a pointer.
