// Local development server for Claude API calls
...
// Extract meaningful content from HTML, handling JavaScript-heavy sites
function extractMeaningfulContent(html) {
  if (!html) return '';
  
  // Use regex-based parsing for server-side content extraction
  
  try {
    // For server-side parsing, we need a different approach
    // Let's extract content using regex patterns for now
...
// API endpoint for restaurant extraction
...
app.listen(PORT, () => {
  ...
  console.log(`📡 Claude API endpoint: http://localhost:${PORT}/api/extract-restaurant`);
});